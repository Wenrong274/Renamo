...
public class TestDeletionRules
{
    #region EveryNth 測試
    [Theory]
    ...
    {
        var rule = DeletionRules.EveryNth(interval, startOffset);
    ...
    {
        var rule = DeletionRules.EveryNth(1);
    ...
    {
        var ex = Assert.Throws<ArgumentOutOfRangeException>(() => DeletionRules.EveryNth(interval));
    ...
    {
        var ex = Assert.Throws<ArgumentOutOfRangeException>(() => DeletionRules.EveryNth(1, -1));
    ...
    {
        var rule = DeletionRules.EveryNth(1, startOffset: 10);
    ...
    public void KeepThenDelete_VariousParameters_ReturnsExpectedIndices(
        int keepCount, int deleteCount, int totalFiles, int[] expectedDeletedIndices)
    {
        var rule = DeletionRules.KeepThenDelete(keepCount, deleteCount);
    ...
    [Fact]
    public void KeepThenDelete_ExactlyOneGroup_DeletesExpectedFiles()
    {
        var rule = DeletionRules.KeepThenDelete(2, 1); // 保留2刪除1，組大小=3
    ...
    [Fact]
    public void KeepThenDelete_IncompleteLastGroup_OnlyDeletesCompleteGroups()
    {
        var rule = DeletionRules.KeepThenDelete(3, 2); // 保留3刪除2，組大小=5
    ...
    [InlineData(0, 1)]
    [InlineData(-1, 1)]
    public void KeepThenDelete_InvalidKeepCount_ThrowsArgumentOutOfRangeException(
        int keepCount, int deleteCount)
    {
        var ex = Assert.Throws<ArgumentOutOfRangeException>(() =>
            DeletionRules.KeepThenDelete(keepCount, deleteCount));
    ...
    [InlineData(1, 0)]
    [InlineData(1, -1)]
    public void KeepThenDelete_InvalidDeleteCount_ThrowsArgumentOutOfRangeException(
        int keepCount, int deleteCount)
    {
        var ex = Assert.Throws<ArgumentOutOfRangeException>(() =>
            DeletionRules.KeepThenDelete(keepCount, deleteCount));
        Assert.Equal("deleteCount", ex.ParamName);
        Assert.Contains("刪除數量必須大於 0", ex.Message);
    }
    ...
    {
        var rule = DeletionRules.EveryNth(1000);
        var deleted = Enumerable.Range(0, 100).Where(rule).ToArray();
        Assert.Empty(deleted);
    }

    [Fact]
    public void KeepThenDelete_LargeKeepCount_WithSmallDataSet_DeletesNothing()
    {
        var rule = DeletionRules.KeepThenDelete(1000, 1);
        var deleted = Enumerable.Range(0, 100).Where(rule).ToArray();
        Assert.Empty(deleted);
    }
    ...
    [Fact]
    public void KeepThenDelete_Keep1Delete1_AlternatesCorrectly()
    {
        var rule = DeletionRules.KeepThenDelete(1, 1);
        var deleted = Enumerable.Range(0, 10).Where(rule).ToArray();
        Assert.Equal(new[] { 1, 3, 5, 7, 9 }, deleted);
    }
    ...
    [Fact]
    public void EveryNth_And_KeepThenDelete_ProduceSameResults_WhenEquivalent()
    {
        // EveryNth(1) 和 KeepThenDelete(1,1) 應該產生相同結果
        var everyNthRule = DeletionRules.EveryNth(1);
        var keepThenDeleteRule = DeletionRules.KeepThenDelete(1, 1);

        var indices = Enumerable.Range(0, 20).ToArray();
        var everyNthResult = indices.Where(everyNthRule).ToArray();
        var keepThenDeleteResult = indices.Where(keepThenDeleteRule).ToArray();

        Assert.Equal(everyNthResult, keepThenDeleteResult);
    }

    [Fact]
    public void EveryNth_And_KeepThenDelete_ProduceSameResults_WhenEquivalent_Complex()
    {
        // EveryNth(2) 和 KeepThenDelete(2,1) 應該產生相同結果
        var everyNthRule = DeletionRules.EveryNth(2);
        var keepThenDeleteRule = DeletionRules.KeepThenDelete(2, 1);
    ...
    public void EveryNth_LogFileCleanup_Scenario()
    {
        ...
        var rule = DeletionRules.EveryNth(2); // 每3個一組，刪除最後一個
    ...
    [Fact]
    public void KeepThenDelete_BackupRotation_Scenario()
    {
        // 模擬備份輪換：每7個備份保留5個，刪除2個
        var rule = DeletionRules.KeepThenDelete(5, 2);
    ...
    public void EveryNth_PhotoThinning_Scenario()
    {
        ...
        var rule = DeletionRules.EveryNth(9); // 每10個一組，刪除第10個
    ...
    [Fact]
    public void KeepThenDelete_EmptySequence_DeletesNothing()
    {
        var rule = DeletionRules.KeepThenDelete(1, 1);
        var deleted = Enumerable.Empty<int>().Where(rule).ToArray();
        Assert.Empty(deleted);
    ...
    {
        var rule = DeletionRules.EveryNth(interval);
    ...
    public void KeepThenDelete_FirstGroup_BehavesCorrectly(
    ...
    {
        var rule = DeletionRules.EveryNth(1000000);
    ...
    [Fact]
    public void KeepThenDelete_VeryLargeNumbers_HandlesCorrectly()
    {
        var rule = DeletionRules.KeepThenDelete(1000000, 1);
    ...
    {
        var everyNthRule = DeletionRules.EveryNth(3); // 每4個一組，刪除第4個
    ...
    {
        var rule1 = DeletionRules.EveryNth(2); // 刪除索引 2,5,8,11,14,17...
        var rule2 = DeletionRules.EveryNth(4); // 刪除索引 4,9,14,19...
    ...
    {
        var rule = DeletionRules.EveryNth(7);
    ...
    [Fact]
    public void KeepThenDelete_RepeatedCalls_ProducesConsistentResults()
    {
        var rule = DeletionRules.KeepThenDelete(3, 2);
    ...
    public void KeepThenDelete_DeletionRatio_IsCorrect(
    ...
    public void EveryNth_OriginalFailingTest_ShouldPass()
    {
        ...
        var rule = DeletionRules.EveryNth(2);
    ...
    public void EveryNth_OriginalPassingTest_ShouldPass()
    {
        ...
        var rule = DeletionRules.EveryNth(100);
    ...
    [Fact]
    public void KeepThenDelete_OriginalTest_Take2Skip1_DeletesOnlyIndex2()
    {
        var rule = DeletionRules.KeepThenDelete(2, 1);
    ...
    [Fact]
    public void KeepThenDelete_OriginalTest_Take1Skip1_Deletes_IndexesOneTwoAndThree()
    {
        var rule = DeletionRules.KeepThenDelete(1, 1);
